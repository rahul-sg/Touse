...
class ZipLgbmPrediction(Base):
    """Latest global-model 12-month price-growth prediction for one ZIP.

    Written by the periodic `train_global_lgbm` Celery task — one row per ZIP.
    The projection service reads this to anchor the Prophet forecast endpoint.
    """
